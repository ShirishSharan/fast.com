!function() {
    ...
    require.alias = function(from, to) {
        ...
        for (var basedir = path.dirname(res), keys = (Object.keys || function(obj) {
            ...
            if (key.slice(0, basedir.length + 1) === basedir + "/") {
                ...
                key === basedir && (require.modules[to] = require.modules[basedir])
        }
    }
    ,
    function() {
        ...
        require.define = function(filename, fn) {
            ...
            var dirname = require._core[filename] ? "" : require.modules.path().dirname(filename)
              , require_ = function(file) {
                ...
                return cached && null === cached.parent && (cached.parent = module_),
                requiredModule
            };
            ...
            require.modules[filename] = function() {
                return require.cache[filename] = module_,
    ...
    require.define("/app/aggregator/stableMovingAverage.js", function(require, module, exports, __dirname, __filename, process, global) {
        module.exports = function(windowSize, snapshotResetThreshold) {
            ...
            function movingAvg(snapshots) {
                snapshots.length < snapshotResetThreshold && reset(),
                lastLen = snapshots.length;
                ...
                if (!fixedStartInd) {
                    ...
                    for (snapshotInd = start; snapshotInd >= end; --snapshotInd)
                        snapshot = snapshots[snapshotInd],
                        sumBytes += snapshot.bytes,
                        sumTime += snapshot.time;
                    speed = sumTime > 0 ? sumBytes / sumTime : 0,
                    speed >= curSpeed ? (startInd = start + 1,
                    curSpeed = speed,
                    bytes = sumBytes,
                    times = sumTime) : fixedStartInd = !0
                }
                for (snapshotInd = startInd; snapshotInd < snapshots.length; ++snapshotInd)
                    snapshot = snapshots[snapshotInd],
                    bytes += snapshot.bytes,
                    times += snapshot.time;
                return startInd = snapshots.length,
                times > 0 ? 1e3 * bytes * 8 / times : 0
            }
            var startInd, curSpeed, fixedStartInd, lastLen, bytes, times;
            return snapshotResetThreshold = snapshotResetThreshold || 5,
            reset(),
            movingAvg
        }
    }),
    require.define("/app/stopper/stableDeltaMeasurementsStopper.js", function(require, module, exports, __dirname, __filename, process, global) {
        module.exports = function(config) {
            function lastWindowMaxInd(measurements, windowSize) {
                var measurement, curMaxSpeed = 0, curMaxInd = 0, measurementInd = 0, numMeasurements = measurements.length, firstMeasurementInd = Math.max(0, numMeasurements - windowSize);
                for (measurementInd = numMeasurements - 1; measurementInd >= firstMeasurementInd; --measurementInd)
                    measurement = measurements[measurementInd],
                    measurement.speed >= curMaxSpeed && (curMaxSpeed = measurement.speed,
                    curMaxInd = measurementInd);
                return curMaxInd
            }
            function maxDelta(value, measurements) {
                var measurementInd, delta, maxDelta = 0;
                for (measurementInd = 0; measurementInd < measurements.length; ++measurementInd)
                    (delta = 100 * Math.abs(measurements[measurementInd].speed - value) / value) > maxDelta && (maxDelta = delta);
                return maxDelta
            }
            function isCompleted(metrics) {
                var maxInd, testTime = void 0 !== metrics.testTime ? metrics.testTime : timer() - startTime, measurements = metrics.progressMeasurements, afterCompleteDuration = metrics.afterCompleteDuration, numMeasurements = measurements.length, curSpeed = metrics.speed;
                return void 0 === afterCompleteDuration ? testTime >= maxDuration || !(numMeasurements < minStableMeasurements) && (maxInd = lastWindowMaxInd(measurements, Math.ceil(minStableMeasurements / 2)),
                !(numMeasurements - maxInd < Math.ceil(minStableMeasurements / 2)) && (maxInd = Math.max(0, numMeasurements - minStableMeasurements),
                !(numMeasurements - maxInd < minStableMeasurements) && (!(maxDelta(curSpeed, measurements.slice(numMeasurements - minStableMeasurements, numMeasurements)) > stabilityDelta) && testTime >= minDuration))) : testTime >= afterCompleteDuration
            }
            var minDuration, maxDuration, stabilityDelta, minStableMeasurements, timer, startTime;
            return minDuration = config.minDuration || 5,
            maxDuration = config.maxDuration || 60,
            stabilityDelta = config.stabilityDelta || 5,
            minStableMeasurements = config.minStableMeasurements || 5,
            timer = config.timer || require("../timer").time,
            startTime = timer(),
            isCompleted
        }
    }),
    require.define("/app/timer.js", function(require, module, exports, __dirname, __filename, process, global) {
        module.exports = function() {
            var timer;
            return this.window && window.performance && (timer = window.performance.now || window.performance.webkitNow) && (timer = timer.bind(window.performance)),
    ...
    require.define("/app/tester.js", function(require, module, exports, __dirname, __filename, process, global) {
        module.exports = function() {
            ...
            return function(requester, config) {
                function validateEvent(eventName, callback) {
                    if (void 0 === events.schema[eventName])
                        throw new errors.TesterEventError("no event with name " + eventName);
                    if (!utils.isFunction(callback))
                        throw new errors.TesterEventError("function callback is expected for event " + eventName)
                }
                function isActive() {
                ...
                function stop() {
                    ...
                    testIsRunning = !1,
                    urlGetter.stop(),
                    reportTimeout && clearTimeout(reportTimeout),
                    retryTimeout && clearTimeout(retryTimeout);
                    for (reqId in requesters)
                        requesters[reqId].stop(),
                        pingers[reqId].stop()
                }
                function reset() {
                    isActive() && stop(),
                    ...
                    requestTimeMs = config.startRangeRequestTimeMs,
                    activeWorkersCount = 0,
                    ocaCount = {},
                    snapshot.reset(),
                    window && window.performance && window.performance.clearResourceTimings && window.performance.clearResourceTimings()
                }
                function computeLoadedLatency() {
                    var reqId, value, latencies = [], count = 0;
                    for (reqId in latencyMeasurements)
                        latencyMeasurements[reqId].length > 4 && (value = utils.percentile(latencyMeasurements[reqId], .75),
                        latencies.push(value)),
                        count += latencyMeasurements[reqId].length;
                    if (0 === latencies.length)
                        for (reqId in latencyMeasurements)
                            value = utils.percentile(latencyMeasurements[reqId], .75),
                            latencies.push(value);
                    return {
                ...
                function computeUnloadedLatency() {
                    var reqId, value, latencies = [], count = 0;
                    for (reqId in latencyMeasurements)
                        value = utils.percentile(latencyMeasurements[reqId], .1),
                        latencies.push(value),
                        count += latencyMeasurements[reqId].length;
                    return {
                        value: Math.min.apply(null, latencies),
                ...
                function reportEndEvents(data) {
                    var eventData, result = data.result, resultValue = -1;
                    eventData = {
                        start: startTime,
                    ...
                    "success" === result ? (eventData.stable = data.stable,
                    currentTestType === TEST_TYPE.LATENCY_TEST ? (resultValue = data.value,
                    eventData.count = data.count,
                    eventData.value = resultValue) : (resultValue = data.speed,
                    eventData.speed = resultValue,
                    eventData.latency = computeLoadedLatency()),
                    event(events.events.ATTEMPT_SUCCESS, eventData),
                    event(events.events.SUCCESS, eventData),
                    eventData.result = result) : "stop" === result ? (eventData.reason = data.reason || "stopped",
                    event(events.events.STOP, eventData),
                    eventData.result = result) : (eventData.error = data.error,
                    eventData.attempts = testAttempt - 1,
                    event(events.events.ATTEMPT_FAIL, eventData),
                    event(events.events.FAIL, eventData),
                    eventData.result = result),
                    event(events.events.ATTEMPT_END, eventData),
                    event(events.events.END, eventData),
                    stats[currentTestType].results.push(resultValue)
                }
                function startTestWorker(workerId, url, attempt) {
                    ...
                    function sendDownloadRequest(onDownloadProgress, onDownloadComplete, maxPayloadSize) {
                        testAttempt < 3 || config.maxBytesInFlight - 1 < activeWorkersCount * maxPayloadSize || !req.supportsProgress() ? rangeUrl = url.replace("/range/", "/range/0-" + requestSize) : (rangeUrl = url.replace("/range/", ""),
                        requestSize = MAX_PAYLOAD_BYTES),
                        event(events.events.CONNECTION_START, {
                            oca: oca,
                            range: requestSize,
                            requestUrl: rangeUrl
                        });
                    ...
                    function sendUploadRequest(onUploadProgress, onUploadComplete, maxPayloadSize) {
                        testAttempt < 3 || config.maxBytesInFlight - 1 < activeWorkersCount * maxPayloadSize || !req.supportsProgress() ? rangeUrl = url.replace("/range/", "/range/0-" + requestSize) : (rangeUrl = url.replace("/range/", ""),
                        requestSize = MAX_PAYLOAD_BYTES),
                        event(events.events.CONNECTION_START, {
                            oca: oca,
                    ...
                    function onProgress(progress) {
                        ...
                        void 0 === progressData.bytes && (progressData.bytes = requestSize),
                        stats[currentTestType].bytes[stats[currentTestType].bytes.length - 1] += progressData.bytes,
                        workerMeasurements[workerId].push(progressData),
                        event(events.events.CONNECTION_PROGRESS, {
                    ...
                    function onComplete(sendRequest) {
                        var maxPayloadSize = MAX_PAYLOAD_BYTES
                          , completeFunc = function(data) {
                            var requestTime, eventData = {
                                oca: oca,
                                requestUrl: rangeUrl
                            }, partialSuccess = !1, ocaRequest = ocaCount[oca] || 0;
                            if (ocaCount[oca] = ocaRequest + 1,
                            maxPayloadSize = MAX_PAYLOAD_BYTES - ocaRequest,
                            workerIsActive() && isActive() && "stopped" !== data.reason) {
                                if (partialSuccess = !data.success && workerMeasurements[workerId].length > 0,
                                !data.success && !partialSuccess) {
                                    var error = new errors.DownloadOcaDataError("Could not download oca data");
                                    return error.url = url,
                                    error.response = data.response,
                                    eventData.error = error,
                                    event(events.events.CONNECTION_FAIL, eventData),
                                    eventData.result = "fail",
                                    event(events.events.CONNECTION_END, eventData),
                                    void test(attempt + 1, error)
                                }
                                eventData.bytes = requestSize,
                                eventData.partialSuccess = partialSuccess,
                                event(events.events.CONNECTION_SUCCESS, eventData),
                                eventData.result = "success",
                                req.supportsProgress() ? requestSize = maxPayloadSize : (requestTime = data.end - data.start,
                                requestSize = Math.min(Math.floor(requestTimeMs * requestSize / requestTime), 5 * requestSize, maxPayloadSize, Math.round(config.maxBytesInFlight / config.connections.max)),
                                requestTimeMs = Math.min(requestTimeMs + config.rangeRequestIncreaseMs, config.maxRangeRequestTimeMs),
                                requestSize = Math.max(requestSize, 1024)),
                                event(events.events.CONNECTION_END, eventData),
                                sendRequest(onProgress, completeFunc, maxPayloadSize)
                            } else
                                req.stop(),
                                eventData.reason = "inactive",
                                eventData.result = "stop",
                                event(events.events.CONNECTION_END, eventData)
                        };
                        return completeFunc
                    }
                    function onLatencyComplete(data) {
                        ...
                        if (workerIsActive() && isActive() && "stopped" !== data.reason) {
                            if (data.success)
                                ...
                                data.timing || (latency /= 2),
                                eventData.latency = latency,
                                eventData.start = data.start,
                                eventData.end = data.end,
                                latencyMeasurements[workerId].push(latency),
                                eventData.timing = data.timing,
                                event(events.events.LATENCY_SUCCESS, eventData),
                                eventData.result = "success";
                            else {
                                ...
                                error.response = data.response,
                                eventData.error = error,
                                event(events.events.LATENCY_FAIL, eventData),
                                eventData.result = "fail"
                            }
                            event(events.events.LATENCY_END, eventData),
                            scheduleDelay = currentTestType !== TEST_TYPE.LATENCY_TEST ? latency ? Math.max(config.latencyMeasurementsFrequencyMs - latency, 0) : config.latencyMeasurementsFrequencyMs : 0,
                            setTimeout(function() {
                    ...
                    var rangeUrl, req = requester(), pinger = requester(), requestSize = config.firstRequestBytes, oca = url.split("/")[2];
                    requesters[workerId] = req,
                    pingers[workerId] = pinger,
                    workerMeasurements[workerId] = [],
                    latencyMeasurements[workerId] = [],
                    req.supportsProgress() && (requestSize = MAX_PAYLOAD_BYTES),
                    config.measureLatency && currentTestType !== TEST_TYPE.LATENCY_TEST && sendLatencyRequest(onLatencyComplete),
                    currentTestType === TEST_TYPE.DOWNLOAD_TEST ? sendDownloadRequest(onProgress, onComplete(sendDownloadRequest), MAX_PAYLOAD_BYTES) : currentTestType === TEST_TYPE.UPLOAD_TEST ? sendUploadRequest(onProgress, onComplete(sendUploadRequest), MAX_PAYLOAD_BYTES) : currentTestType === TEST_TYPE.LATENCY_TEST && sendLatencyRequest(onLatencyComplete)
                }
                function reportMetrics(interval) {
                    function testTime() {
                        return (timer() - (downloadStartTime || startTime)) / 1e3
                    }
                    ...
                    if (isActive())
                        if (currentTestType === TEST_TYPE.LATENCY_TEST)
                            resultData = computeUnloadedLatency(),
                            resultData.stable = !0,
                            testComplete = testTime() > 5 && resultData.count > 50,
                            testComplete ? (stop(),
                            resultData.result = "success",
                            setTimeout(function() {
                                reportEndEvents(resultData)
                            }, 10)) : (event(events.events.PROGRESS, resultData),
                            reportTimeout = setTimeout(function() {
                                reportMetrics(interval)
                            }, interval));
                        else {
                            latestSnapshot = snapshot.compute(workerMeasurements),
                            latestSnapshot && (activeSnapshot = latestSnapshot.end - latestSnapshot.start > 0,
                            event(events.events.SNAPSHOT, {
                            ...
                            testTimeS = testTime(),
                            activeSnapshot && (currentSpeed = speedAggregator(snapshots),
                            testComplete = testTimeS > config.duration.min && stopper({
                                testTime: testTimeS,
                                snapshots: snapshots,
                                progressMeasurements: progressMeasurements,
                                speed: currentSpeed
                            })),
                            testTimeS > config.duration.max && (testComplete = !0,
                            currentSpeed = speedAggregator(snapshots)),
                            resultData = {
                                speed: currentSpeed,
                                stable: !0,
                                bytes: stats[currentTestType].bytes[stats[currentTestType].bytes.length - 1],
                            ...
                            testComplete ? (testTimeS > config.duration.max && (resultData.stopperStable = !1,
                            progressMeasurements.length > 10 && (resultData.stable = !0)),
                            stop(),
                            resultData.result = "success",
                            setTimeout(function() {
                                reportEndEvents(resultData)
                            ...
                            if (activeWorkersCount < config.connections.max)
                                for (currentSpeed > 5e7 ? desiredWorkers = config.connections.max : currentSpeed > 1e7 && activeWorkersCount < 5 ? desiredWorkers = 5 : currentSpeed > 1e6 && activeWorkersCount < 3 && (desiredWorkers = 3),
                                currentSpeed > 5e5 && activeWorkersCount < 2 && (desiredWorkers = 3),
                                i = 0; i < desiredWorkers - activeWorkersCount; ++i)
                                    startNewWorker()
                        }
                }
                ...
                function startNewWorker() {
                    function startForUrl(url) {
                        ...
                        downloadStartTime = downloadStartTime || timer(),
                        startTestWorker(id, url, testAttempt)
                    }
                    function restartTest(error) {
                        return urlRequestFailure(error)
                    }
                    activeWorkersCount += 1,
                    event(events.events.URL_REQUEST_START, config.getTestOcasParams),
                    urlGetter.getNext(config.getTestOcasParams, requester, startForUrl, restartTest, urlRequestStop)
                }
                function test(attempt, error) {
                    function startTest() {
                        ...
                        event(events.events.ATTEMPT_START, eventData),
                        startTime = timer();
                        var i;
                        if (currentTestType !== TEST_TYPE.LATENCY_TEST)
                            for (i = 0; i < config.connections.min; i++)
                                startNewWorker();
                        else
                            for (i = 0; i < Math.min(5, config.connections.max); i++)
                                startNewWorker();
                        reportMetrics(config.progressFrequencyMs)
                    }
                    var eventData;
                    if (reset(),
                    (testAttempt = attempt || 1) > config.maxAttempts)
                        return void reportEndEvents({
                            result: "fail",
                            error: error
                        });
                    testIsRunning = !0,
                    ...
                    eventData.result = "fail",
                    event(events.events.ATTEMPT_END, eventData),
                    error.url && urlGetter.reportBadUrl(error.url)),
                    testAttempt > 1 ? setTimeout(startTest, Math.min(Math.pow(2, testAttempt), 200)) : startTest()
                }
                function event(eventName, data) {
                    var eventInd, genericHandlers, handlers = eventHandlers[eventName];
                    ...
                    if ((genericHandlers = eventHandlers[events.events.ANY]) && genericHandlers.length > 0)
                        for (eventInd = 0; eventInd < genericHandlers.length; ++eventInd)
                            genericHandlers[eventInd]({
                                event: eventName,
                                data: utils.simpleCopy(data)
                            })
                }
                var that, currentTestType, reportTimeout, retryTimeout, requestTimeMs, activeWorkersCount, speedAggregator, stopper, timer, testType, testIsRunning = !1, testAttempt = 1, stats = {}, startTime = null, downloadStartTime = null, workerMeasurements = {}, latencyMeasurements = {}, requesters = {}, pingers = {}, progressMeasurements = [], urlGetter = require("./url_getter")(), uniqueId = utils.uniqueId, snapshot = require("./snapshot"), eventHandlers = {}, ocaCount = {};
                for (testType in TEST_TYPE)
                    testType = TEST_TYPE[testType],
                    stats[testType] = {},
                    stats[testType].tests = 0,
                    stats[testType].results = [],
                    stats[testType].bytes = [];
                if (!requester)
                    throw new errors.TesterArgumentError("requester factory should be provided to the tester");
                return config = testConfig.get(config),
                timer = config.timer,
                speedAggregator = config.aggregator,
                stopper = config.stopper,
                that = {
                    download: function() {
                        currentTestType = TEST_TYPE.DOWNLOAD_TEST,
                        stats[currentTestType].tests += 1,
                        stats[currentTestType].bytes.push(0),
                        urlGetter.reset(),
                        event(events.events.START, {
                            numberOfTests: stats[currentTestType].tests,
                    ...
                    upload: function() {
                        currentTestType = TEST_TYPE.UPLOAD_TEST,
                        stats[currentTestType].tests > 1 && urlGetter.reset(),
                        stats[currentTestType].tests += 1,
                        stats[currentTestType].bytes.push(0),
                        event(events.events.START, {
                            numberOfTests: stats[currentTestType].tests,
                    ...
                    latency: function() {
                        currentTestType = TEST_TYPE.LATENCY_TEST,
                        stats[currentTestType].tests > 1 && urlGetter.reset(),
                        stats[currentTestType].tests += 1,
                        stats[currentTestType].bytes.push(0),
                        event(events.events.START, {
                    ...
                    on: function(eventName, callback) {
                        ...
                        return handlers || (handlers = eventHandlers[eventName] = []),
                        handlers.push(callback),
                        that
                    },
                    off: function(eventName, callback) {
                        validateEvent(eventName, callback);
                        var eventInd, event, handlers = eventHandlers[eventName], outHandlers = [];
                        if (handlers && handlers.length > 0) {
                            for (eventInd = 0; eventInd < handlers.length; ++eventInd)
                                (event = handlers[eventInd]) !== callback && outHandlers.push(event);
                    ...
                    setConfig: function(attrs) {
                        for (var name in attrs)
                            testConfig.validate(name, attrs[name]) && (config[name] = attrs[name]);
                        return config
                    },
                    isRunning: function() {
                        return testIsRunning
    ...
    require.define("/app/utils.js", function(require, module, exports, __dirname, __filename, process, global) {
        var utils = {};
        ...
        utils.median = function(values) {
            ...
            if (0 !== (l = values.length))
                return 1 === l ? values[0] : (half = Math.floor(l / 2),
                values.sort(),
                l % 2 ? values[half] : (values[half - 1] + values[half]) / 2)
        }
        ,
        ...
        utils.onDomReady = function(callback) {
            function onReadyIe(event) {
                "complete" === document.readyState && (document.detachEvent("onreadystatechange", onReadyIe),
                callback())
            }
        ...
        utils.genBlob = function(size) {
            var blobStr;
            for (blobStr = ""; 2 * blobStr.length <= size; )
                blobStr += blobStr + Math.random().toString();
    ...
    require.define("/app/error.js", function(require, module, exports, __dirname, __filename, process, global) {
        function TesterArgumentError() {
            var temp = Error.apply(this, arguments);
            temp.name = this.name = "TesterArgumentError",
            this.stack = temp.stack,
            this.message = temp.message
        }
        function TesterEventError() {
            var temp = Error.apply(this, arguments);
            temp.name = this.name = "TesterEventError",
            this.stack = temp.stack,
            this.message = temp.message
        }
        function RequesterArgumentError() {
            var temp = Error.apply(this, arguments);
            temp.name = this.name = "RequesterArgumentError",
            this.stack = temp.stack,
            this.message = temp.message
        }
        function GetOcaUrlError() {
            var temp = Error.apply(this, arguments);
            temp.name = this.name = "GetOcaUrlError",
            this.stack = temp.stack,
            this.message = temp.message
        }
        function NoOcaUrlsError() {
            var temp = Error.apply(this, arguments);
            temp.name = this.name = "NoOcaUrlsError",
            this.stack = temp.stack,
            this.message = temp.message
        }
        function DownloadOcaDataError() {
            var temp = Error.apply(this, arguments);
            temp.name = this.name = "DownloadOcaDataError",
            this.stack = temp.stack,
            this.message = temp.message
        }
        var errors = {};
        TesterArgumentError.prototype = Error.prototype,
        errors.TesterArgumentError = TesterArgumentError,
        TesterEventError.prototype = Error.prototype,
        errors.TesterEventError = TesterEventError,
        RequesterArgumentError.prototype = Error.prototype,
        errors.RequesterArgumentError = RequesterArgumentError,
        GetOcaUrlError.prototype = Error.prototype,
        errors.GetOcaUrlError = GetOcaUrlError,
        NoOcaUrlsError.prototype = Error.prototype,
        errors.NoOcaUrlsError = NoOcaUrlsError,
        DownloadOcaDataError.prototype = Error.prototype,
        errors.DownloadOcaDataError = DownloadOcaDataError,
        module.exports = errors
    }),
    require.define("/app/event.js", function(require, module, exports, __dirname, __filename, process, global) {
        var events = {
            START: "start",
            END: "end",
            SUCCESS: "success",
    ...
    require.define("/app/config.js", function(require, module, exports, __dirname, __filename, process, global) {
        module.exports = function() {
            function positiveNumberValidator(name) {
                return function(value) {
                    if (value <= 0)
                        throw new errors.TesterArgumentError(name + ": expected value should be above 0, given value: " + value)
                }
            }
            ...
            function validate(name, value) {
                var validator;
                if (void 0 === defaultConfig[name])
                    throw new errors.TesterArgumentError("unknown tester config attribute: " + name);
                return validator = validators[name],
              ...
              , validators = {
                ...
                protocol: function(protocol) {
                    if ("http" !== protocol && "https" !== protocol)
                        throw new errors.TesterArgumentError("protocol: only http and https are supported, given value: " + protocol)
                },
                version: function(version) {
                    throw new errors.TesterArgumentError("can not override tester version")
                },
                duration: function(duration) {
                    if (void 0 === duration.min)
                        throw new errors.TesterArgumentError("duration: when provided, duration.min is mandatory");
                    if (void 0 === duration.max)
                        throw new errors.TesterArgumentError("duration: when provided, duration.max is mandatory");
                    if (duration.min > duration.max)
                        throw new errors.TesterArgumentError("duration: max duration should be higher or equal to min duration, given value: min=" + duration.min + ", max=" + duration.max);
                    if (duration.min < 0)
                        throw new errors.TesterArgumentError("duration.min can not be negative");
                    if (duration.max < 0)
                        throw new errors.TesterArgumentError("duration.max can not be negative");
                    if (duration.max > 300)
                        throw new errors.TesterArgumentError("duration.max is limited at 300")
                },
                connections: function(connections) {
                    if (void 0 === connections.min)
                        throw new errors.TesterArgumentError("connections: when provided, connections.min is mandatory");
                    if (void 0 === connections.max)
                        throw new errors.TesterArgumentError("connections: when provided, connections.max is mandatory");
                    if (connections.min > connections.max)
                        throw new errors.TesterArgumentError("connections: max connections should be higher or equal to min connections, given value: min=" + connections.min + ", max=" + connections.max);
                    if (connections.min <= 0)
                        throw new errors.TesterArgumentError("connections.min should be positive");
                    if (connections.max <= 0)
                        throw new errors.TesterArgumentError("connections.max should be positive");
                    if (connections.max > 30)
                        throw new errors.TesterArgumentError("connections.max is limited at 30")
                }
            }
              , defaultDuration = {
                min: 5,
            ...
            return {
                get: function(overrides) {
                    ...
                    return overrides && update(config, overrides),
                    config
                },
    ...
    require.define("/app/aggregator/movingAverage.js", function(require, module, exports, __dirname, __filename, process, global) {
        module.exports = function(windowSize) {
            function movingAvg(snapshots) {
                var snapshotInd, bytes = 0, times = 0, start = snapshots.length - 1, end = Math.max(0, snapshots.length - windowSize);
                for (snapshotInd = start; snapshotInd >= end; --snapshotInd)
                    snapshot = snapshots[snapshotInd],
                    bytes += snapshot.bytes,
                    times += snapshot.time;
    ...
    require.define("/app/stopper/stableMeasurementsStopper.js", function(require, module, exports, __dirname, __filename, process, global) {
        module.exports = function(config) {
            function lastWindowMaxInd(measurements, windowSize) {
                var measurement, curMaxSpeed = 0, curMaxInd = 0, measurementInd = 0, numMeasurements = measurements.length, firstMeasurementInd = Math.max(0, numMeasurements - windowSize);
                for (measurementInd = numMeasurements - 1; measurementInd >= firstMeasurementInd; --measurementInd)
            ...
            function isCompleted(metrics) {
                ...
                if (void 0 === afterCompleteDuration) {
                    if (testTime >= maxDuration)
                        return !0;
                    if (numMeasurements < minStableMeasurements)
                        return !1;
                    var maxInd = lastWindowMaxInd(measurements, minStableMeasurements);
                    if (numMeasurements - maxInd < minStableMeasurements)
                        return !1;
                    return !(maxDelta(measurements[maxInd].speed, measurements.slice(numMeasurements - minStableMeasurements, numMeasurements)) > stabilityDelta) && testTime >= minDuration
                }
    ...
    require.define("/node_modules/circular-buffer/package.json", function(require, module, exports, __dirname, __filename, process, global) {
        module.exports = {
            main: "index.js"
        }
    }),
    require.define("/node_modules/circular-buffer/index.js", function(require, module, exports, __dirname, __filename, process, global) {
        function CircularBuffer(capacity) {
            ...
            if ("object" == typeof capacity && Array.isArray(capacity._buffer) && "number" == typeof capacity._capacity && "number" == typeof capacity._first && "number" == typeof capacity._size)
                for (var prop in capacity)
                    capacity.hasOwnProperty(prop) && (this[prop] = capacity[prop]);
            else {
                if ("number" != typeof capacity || capacity % 1 != 0 || capacity < 1)
        ...
        CircularBuffer.prototype = {
            ...
            enq: function(value) {
                this._first > 0 ? this._first-- : this._first = this._capacity - 1,
                this._buffer[this._first] = value,
                this._size < this._capacity && this._size++
            },
            push: function(value) {
                this._size == this._capacity ? (this._buffer[this._first] = value,
                this._first = (this._first + 1) % this._capacity) : (this._buffer[(this._first + this._size) % this._capacity] = value,
                this._size++)
            },
            deq: function() {
                if (0 == this._size)
            ...
            shift: function() {
                ...
                return this._first == this._capacity - 1 ? this._first = 0 : this._first++,
                this._size--,
                value
            },
            get: function(start, end) {
                if (0 == this._size && 0 == start && (void 0 == end || 0 == end))
                    return [];
                if ("number" != typeof start || start % 1 != 0 || start < 0)
                ...
                if ("number" != typeof end || end % 1 != 0 || end < 0)
                    throw new TypeError("Invalid end");
                if (end >= this._size)
                    throw new RangeError("Index past end of buffer: " + end);
                return this._first + start >= this._capacity && (start -= this._capacity,
                end -= this._capacity),
                this._first + end < this._capacity ? this._buffer.slice(this._first + start, this._first + end + 1) : this._buffer.slice(this._first + start, this._capacity).concat(this._buffer.slice(0, this._first + end + 1 - this._capacity))
            },
            toarray: function() {
    ...
    require.define("/app/url_getter.js", function(require, module, exports, __dirname, __filename, process, global) {
        var urlGetter = function() {
            function parseSpeedTestUrls(responseText) {
                ...
                clientInfo.servers = [],
                ind = 0; ind < targets.length; ++ind)
                    url = targets[ind].url,
                    url = url.replace(/speedtest/, "speedtest/range/"),
                    urlReporter.isBadUrl(url) || (testUrls.push(url),
                    clientInfo.servers.push(targets[ind])),
                    allUrls.push(url);
                return 0 === testUrls.length && (testUrls = allUrls),
                testUrls
            }
            function formatUrl(params) {
                var param, endpoint = params.endpoint || DEFAULT_PARAMS.endpoint, https = void 0 !== params.https ? params.https : DEFAULT_PARAMS.https, token = params.token || DEFAULT_PARAMS.token, urlCount = params.urlCount || DEFAULT_PARAMS.urlCount, url = "https://" + endpoint + "?https=" + https + "&token=" + token + "&urlCount=" + urlCount;
                for (param in params.extraParams)
                    params.extraParams.hasOwnProperty(param) && (url += "&" + param + "=" + params.extraParams[param]);
                return url
            }
            function reset() {
                testUrls = [],
            ...
            var DEFAULT_PARAMS, curUrlInd, testUrls, that, errors, req, urlReporter, clientInfo, waitOnRefresh;
            return errors = require("./error"),
            dummy = require("./utils").dummy,
            DEFAULT_PARAMS = {
                https: !0,
            ...
            that = {},
            urlReporter = function() {
                function parseUrl(url) {
                    var endpoint, urlParts = url.split("?"), urlData = {};
                    return urlData.url = url,
                    urlData.normalizedUrl = url,
                    endpoint = urlParts[0],
                    2 === urlParts.length && (speedtestInd = endpoint.lastIndexOf("/speedtest"),
                    speedtestInd ? (endpoint = endpoint.substring(0, speedtestInd + 10),
                    urlData.normalizedUrl = endpoint + "?" + urlParts[1]) : urlData.normalizedUrl = url),
                    endpoint = endpoint.replace(/.*?:\/\//g, ""),
                    urlData.oca = endpoint.split("/")[0],
                    urlData.site = urlData.oca.split(".")[2],
                    urlData
                }
                function reset() {
                    badUrls = {},
                ...
                return reset(),
                {
                    ...
                    isBadUrl: function(url) {
                        ...
                        return url = urlData.normalizedUrl,
                        void 0 !== badUrls[url] || ocaFails >= 2 || siteFails >= 3
                    },
                    reportBadUrl: function(url) {
                        var urlData = parseUrl(url);
                        url = urlData.normalizedUrl,
                        badUrls[url] = !0,
                        urlData.oca && (ocaFailures[urlData.oca] = void 0 === ocaFailures[urlData.oca] ? 1 : ocaFailures[urlData.oca] + 1),
                        1 === ocaFailures[urlData.oca] && urlData.site && (siteFailures[urlData.site] = void 0 === siteFailures[urlData.site] ? 1 : siteFailures[urlData.site] + 1)
                    }
                }
            }(),
            reset(),
            that.get = function(urlParams, requester, onComplete, onFail, onStop, refresh) {
                ...
                if (0 === testUrls.length || refresh)
                    return void that.refresh(urlParams, requester, returnUrl, onFail, onStop);
                returnUrl(testUrls)
            }
            ,
            that.getNext = function(urlParams, requester, onComplete, onFail, onStop, refresh) {
                ...
                function waitTillRefresh(callback) {
                    function waitMore() {
                        waitTillRefresh(callback)
                    }
                    waitOnRefresh ? setTimeout(waitMore, 50) : callback()
                }
                function getUrl() {
                    curUrlInd = (curUrlInd + 1) % testUrls.length,
                    testUrls.length > 0 ? onComplete(testUrls[curUrlInd]) : onFail(new errors.NoOcaUrlsError("No urls returned from the endpoint " + urlParams))
                }
                testUrls.length < 3 || refresh ? waitOnRefresh ? waitTillRefresh(getUrl) : (waitOnRefresh = !0,
                that.get(urlParams, requester, wrapper(onComplete), wrapper(onFail), wrapper(onStop), !0)) : getUrl()
            }
            ,
            that.reportBadUrl = function(url) {
                var urlInd, newTestUrls = [];
                for (urlReporter.reportBadUrl(url),
                urlInd = 0; urlInd < testUrls.length; ++urlInd)
                    urlReporter.isBadUrl(testUrls[urlInd]) || newTestUrls.push(testUrls[urlInd]);
            ...
            that.refresh = function(urlParams, requester, onSuccess, onFail, onStop) {
                function urlSuccess(data) {
                    req = void 0,
                    data.success ? (reset(),
                    testUrls = parseSpeedTestUrls(data.response),
                    onSuccess(testUrls)) : "stopped" !== data.reason ? (error = new errors.GetOcaUrlError("Could not send request to " + url),
                    error.response = data.response,
                    onFail(error)) : onStop(data)
                }
                req = requester(),
                url = formatUrl(urlParams),
                req.start(url, dummy, urlSuccess, !0)
            }
            ,
            that.stop = function() {
                req && req.stop()
    ...
    require.define("/app/snapshot.js", function(require, module, exports, __dirname, __filename, process, global) {
        module.exports = function() {
            function reset() {
                snapshots = [],
                lastProcessedWorkerMeasurement = {},
            ...
            function getLatestSnapshotMeasurements(workerMeasurements) {
                ...
                newLastProcessedWorkerMeasurement = {};
                for (workerId in workerMeasurements)
                    if (workerMeasurements.hasOwnProperty(workerId)) {
                        if (curWorkerMeasurements = workerMeasurements[workerId],
                        workerMeasurementsCount = curWorkerMeasurements.length,
                        lastMeasurementInd = lastProcessedWorkerMeasurement[workerId] || 0,
                        curWorkerOverflowMeasurements = overflowMeasurements[workerId] || [],
                        !(lastMeasurementInd < workerMeasurementsCount || curWorkerOverflowMeasurements.length > 0))
                            return;
                        overflowMeasurements[workerId] = [],
                        ...
                        lastMeasurement = curWorkerMeasurements[workerMeasurementsCount - 1],
                        (!snapshotEnd || lastMeasurement.end < snapshotEnd) && (snapshotEnd = lastMeasurement.end),
                        newLastProcessedWorkerMeasurement[workerId] = workerMeasurementsCount
                    }
            ...
            function trimSnapshotMeasurements(snapshotMeasurements, snapshotEnd) {
                ...
                for (snapshotInd = 0; snapshotInd < snapshotMeasurements.length; ++snapshotInd) {
                    for (snapshot = snapshotMeasurements[snapshotInd],
                    curWorkerMeasurements = snapshot.measurements,
                    workerId = snapshot.workerId,
                    measurementId = curWorkerMeasurements.length - 1; measurementId >= 0 && (lastMeasurement = curWorkerMeasurements[measurementId],
                    lastMeasurement.start >= snapshotEnd); --measurementId)
                        overflowMeasurements[workerId] || (overflowMeasurements[workerId] = []),
                        overflowMeasurements[workerId].push(lastMeasurement);
                    snapshot.measurements = curWorkerMeasurements.slice(0, measurementId + 1),
                    measurementId >= 0 && lastMeasurement && lastMeasurement.end > snapshotEnd && (overflowMeasurement = {
                        start: snapshotEnd,
                        end: lastMeasurement.end
                    },
                    overflowMeasurement.bytes = lastMeasurement.bytes * (lastMeasurement.end - snapshotEnd) / (lastMeasurement.end - lastMeasurement.start),
                    lastMeasurement.bytes -= overflowMeasurement.bytes,
                    lastMeasurement.end = snapshotEnd,
                    overflowMeasurements[workerId] || (overflowMeasurements[workerId] = []),
                    overflowMeasurements[workerId].push(overflowMeasurement))
                }
            ...
            function makeSnapshot(snapshotMeasurements) {
                ...
                curRange = {},
                rangeFinished = 0; rangeFinished < snapshotMeasurements.length; ) {
                    var curMin = function(curWorkerMeasurements) {
                        var snapshotInd, min = {
                            workerId: null,
                            measurement: {},
                            snapshotInd: null
                        ...
                        for (snapshotInd = 0; snapshotInd < snapshotMeasurements.length; ++snapshotInd) {
                            var workerId = snapshotMeasurements[snapshotInd].workerId
                              ...
                              , measurement = measurements[measurementInd];
                            measurement && (void 0 === min.measurement.start || measurement.start < min.measurement.start) && (min.workerId = workerId,
                            min.measurement = measurement,
                            min.snaphotInd = snapshotInd)
                        }
                        return min
                    }(curRange);
                    null !== curMin.workerId ? (curRange[curMin.workerId] = (curRange[curMin.workerId] || 0) + 1,
                    curRange[curMin.workerId] >= snapshotMeasurements[curMin.snaphotInd].measurements.length && (rangeFinished += 1),
                    void 0 === minTime && (minTime = curMin.measurement.start,
                    curLastTime = minTime),
                    curLastTime = Math.max(curLastTime, curMin.measurement.start),
                    curMin.measurement.end > curLastTime && (snapshot.time += curMin.measurement.end - curLastTime,
                    curLastTime = curMin.measurement.end),
                    snapshot.bytes += curMin.measurement.bytes) : rangeFinished += 1
                }
                if (void 0 !== (maxTime = curLastTime) && void 0 !== minTime)
                    return snapshot.start = minTime,
                    snapshot.end = maxTime,
                    snapshot
            }
            ...
            return reset(),
            {
                ...
                compute: function(workerMeasurements) {
                    ...
                    if (workerSnapshotData) {
                        ...
                        return snapshot && snapshots.push(snapshot),
                        snapshot
                    }
                },
                length: function() {
    ...
    require.define("/app/requester/xhr.js", function(require, module, exports, __dirname, __filename, process, global) {
        var requester = function() {
            function extractTimingInfo(data, url) {
                var entries, timing, end;
                window && window.performance && window.performance.getEntriesByName && (entries = window.performance.getEntriesByName(url),
                0 != entries.length && (timing = entries[entries.length - 1],
                (end = timing.responseStart || timing.responseEnd) && (data.timing = timing,
                data.start = timing.requestStart || timing.connectEnd || timing.startTime || timing.fetchTime || data.start,
                data.end = end)))
            }
            function validate(url, onProgressCallback, onCompleteCallback) {
                if (!url)
                    throw new errors.RequesterArgumentError("url arguments is mandatory to perform speed test");
                if (!onProgressCallback)
                    throw new errors.RequesterArgumentError("onProgressCallback is mandatory");
                if (!onCompleteCallback)
                    throw new errors.RequesterArgumentError("onCompleteCallback is mandatory")
            }
            function startTest(url, withResponse) {
                ...
                function onerror() {
                    ...
                    !errorMessage && withResponse && (errorMessage = this.responseText),
                    this.getAllResponseHeaders && (headers = this.getAllResponseHeaders()),
                    onComplete({
                        success: !1,
                        response: {
                            type: "RequestError",
                ...
                function onload() {
                    ...
                    withResponse && (data.response = this.responseText),
                    responseSize = this.response && this.response.size,
                    extractTimingInfo(data, url),
                    onProgress({
                ...
                function onprogress(e) {
                    ...
                    200 !== this.status && 304 !== this.status || !e.lengthComputable || (now = timer(),
                    newBytesLoaded = e.loaded - lastBytesLoaded,
                ...
                xmlHttpTimeout = setTimeout(ontimeout, requestTimeoutMs),
                "onreadystatechange"in request ? request.onreadystatechange = function(e) {
                ...
                withResponse || (request.overrideMimeType && request.overrideMimeType("text/plain; charset=x-user-defined"),
                request.responseType = "blob"),
                lastProgressTime = lastCompleteTime = timer(),
                request.timeout = 6e4,
                setTimeout(function() {
            ...
            function uploadTest(url, size) {
                ...
                function onerror() {
                    ...
                    clearTimeout(xmlHttpTimeout),
                    errorMessage = this.response,
                    errorMessage || (errorMessage = this.responseText),
                    this.getAllResponseHeaders && (headers = this.getAllResponseHeaders()),
                    onComplete({
                        success: !1,
                        response: {
                ...
                lastBytesLoaded = 0,
                request = xhr(),
                request.upload && (request.upload.onprogress = onprogress),
                xmlHttpTimeout = setTimeout(ontimeout, requestTimeoutMs),
                "onreadystatechange"in request ? request.onreadystatechange = function(e) {
                    testIsRunning && 4 === this.readyState && (this.status >= 200 && this.status < 300 || 304 === this.status ? onload.apply(this) : onerror.apply(this))
                }
                : request.upload && (request.upload.onerror = onerror,
                request.upload.onload = onload,
                ...
                var blob = null;
                size > 0 && (blob = utils.genBlob(size)),
                lastProgressTime = lastCompleteTime = timer(),
                request.timeout = 6e4,
                setTimeout(function() {
            ...
            return requestTimeoutMs = 1e4,
            errors = require("../error"),
            utils = require("../utils"),
            xhr = utils.getXHR,
            timer = require("../timer").time,
            dummy = require("../utils").dummy,
            testIsRunning = !1,
            {
                start: function(url, onProgressCallback, onCompleteCallback, withResponse) {
                    validate(url, onProgressCallback, onCompleteCallback),
                    onComplete = onCompleteCallback,
                    onProgress = onProgressCallback,
                    testIsRunning = !0,
                    startTest(url, withResponse)
                },
                ...
                supportsProgress: function() {
                    if (void 0 !== supportsProgress)
                        return supportsProgress;
                    supportsProgress = (!window.XDomainRequest || -1 !== navigator.appVersion.indexOf("MSIE 10")) && "onprogress"in xhr()
                },
                stop: function() {
                    testIsRunning = !1,
                    request && (request.onreadystatechange = dummy,
                    request.onprogress = dummy,
                    request.abort(),
                    request = void 0,
                    onComplete({
                        success: !1,
    ...
    require.define("/app/logger.js", function(require, module, exports, __dirname, __filename, process, global) {
        function logger(tester, config) {
            function sendLogs(data) {
                function onload() {}
                function onerror() {}
                var request = require("./utils").getXHR();
                request.onload ? (request.onload = onload,
                request.onerror = onerror) : request.onreadystatechange = function(e) {
                    4 === this.readyState && this.status >= 200 && this.status
                }
                ,
                request.open("POST", logUrl, !0),
                request.timeout = 5e3,
                request.setRequestHeader && request.setRequestHeader("Content-type", "application/json"),
                request.send(data)
            }
            ...
            function endConnectionSession(requestUrl, data) {
                var connectionSessionId = connectionSessionIds[requestUrl];
                data.type = endSessionType(data),
                data.measurements = connectionEvents[requestUrl],
                nfLogger.endSession(connectionSessionId, data),
                delete connectionSessionIds[requestUrl],
                delete connectionEvents[requestUrl]
            }
            function endMeasureSession(sessionId, data) {
                data.type = endSessionType(data),
                data.snapshots = snapshotEvents,
                data.progress = progressEvents,
                data.latencies = latencyEvents,
                snapshotEvents = [],
                progressEvents = [],
                latencyEvents = {},
                nfLogger.endSession(sessionId, data)
            }
            PAGE_LOAD = "pageLoad";
            var logUrl, logSource, nfLogger, testSessionId, testAttemptSessionId, afterCompleteSessionId, loggingEnabled = !1, connectionSessionIds = {}, getUrlSessionIds = [], snapshotEvents = [], progressEvents = [], latencyEvents = {}, connectionEvents = {}, events = require("./event").events;
            return config = config || {},
            logUrl = config.url || "https://ichnaea.test.netflix.com/cl2",
            logSource = config.source || "netspeed",
            require("./utils"),
            nfLogger = require("nf-cl-logger")({
                requestSender: sendLogs,
                batchInterval: 6e5,
                batchSize: 1e5,
                source: logSource
            }),
            nfLogger.addContext("Client", {
            ...
            }),
            {
                startLogging: function() {
                    loggingEnabled || (loggingEnabled = !0,
                    tester.on(events.START, function(data) {
                        testSessionId = nfLogger.startSession("SpeedTest", data)
                    }),
                    tester.on(events.END, function(data) {
                        data[PAGE_LOAD] = getPageLoadPerformance(),
                        data.type = endSessionType(data),
                        nfLogger.endSession(testSessionId, data),
                        nfLogger.flush()
                    }),
                    tester.on(events.ATTEMPT_START, function(data) {
                        testAttemptSessionId = nfLogger.startSession("Measure", data)
                    }),
                    ...
                    tester.on(events.CONNECTION_START, function(data) {
                        var request = data.requestUrl;
                        connectionSessionIds[request] = nfLogger.startSession("Connection", data),
                        connectionEvents[request] = []
                    }),
                    tester.on(events.CONNECTION_PROGRESS, function(data) {
                        var requestUrl = data.requestUrl;
                        connectionEvents[requestUrl] && connectionEvents[requestUrl].push({
                            bytes: data.bytes,
                            start: data.start,
                            end: data.end
                        })
                    ...
                    tester.on(events.CONNECTION_END, function(data) {
                        endConnectionSession(data.requestUrl, data)
                    }),
                    tester.on(events.URL_REQUEST_START, function(data) {
                        getUrlSessionIds.push(nfLogger.startSession("GetUrl", data))
                    }),
                    tester.on(events.URL_REQUEST_END, function(data) {
                        var sessionId = getUrlSessionIds[0];
                        sessionId && (data.type = endSessionType(data),
                        nfLogger.endSession(sessionId, data),
                        getUrlSessionIds.shift())
                    }),
                    tester.on(events.LATENCY_END, function(data) {
                        var oca = data.oca;
                        latencyEvents[oca] || (latencyEvents[oca] = []),
                        latencyEvents[oca].push(data)
                    }),
                    ...
                    tester.on(events.AFTER_COMPLETE_ATTEMPT_START, function(data) {
                        afterCompleteSessionId = nfLogger.startSession("AfterCompleteMeasure", data)
                    }),
                    tester.on(events.AFTER_COMPLETE_ATTEMPT_END, function(data) {
                        endMeasureSession(afterCompleteSessionId, data),
                        nfLogger.flush()
                    }),
    ...
    require.define("/node_modules/nf-cl-logger/src/logger.js", function(require, module, exports, __dirname, __filename, process, global) {
        ...
        function createCompactLogger(optionsArg) {
            ...
            return options.version = options.version || "2.0",
    ...
    require.define("/node_modules/nf-cl-logger/src/logger-core.js", function(require, module, exports, __dirname, __filename, process, global) {
        ...
        var SCHEMA = require("nf-cl-schema-ui")
          , INCREMENTING_BITS_MASK = Math.pow(2, 28) - 1
          , RANDOM_BITS_SHIFT = Math.pow(2, 25);
        Logger.prototype = {
            constructor: Logger,
            batchInterval: 3e4,
            batchSize: 50,
            ...
            addContext: function(type, data) {
                ...
                return this._state.pending[context.id] = context,
                context.id
            },
            removeContext: function(id) {
                return this._state.pending[id] ? (delete this._state.pending[id],
                id) : this._state.current[id] ? (this._state.currentDelta.push(this._state.current[id]),
                delete this._state.current[id],
                id) : null
            },
            ...
            startSession: function(type, data) {
                ...
                return this._state.current[context.id] = context,
                this._snapshot(),
                context.id
            },
            endSession: function(sessionId, data) {
                ...
                if (startContext) {
                    ...
                    return endContext.duration = endContext.time - startContext.time,
                    endContext.sessionId = sessionId,
                    delete this._state.current[sessionId],
                    this._snapshot(endContext, startContext),
                    sessionId
                }
                return null
            },
            flush: function() {
                ...
                if (!state.ending && state.snapshots && state.snapshots.length) {
                    ...
                    state.snapshots = [],
                    this.requestSender(JSON.stringify(envelope))
                }
            },
            serialize: function() {
                ...
                return this._batchTimeout = timer,
                json
            },
            sever: function(severedContext) {
                this.end(severedContext || "Severed"),
                this._init(this)
            },
            end: function(endingContext) {
                endingContext && this.addContext(endingContext),
                this._state.ending = !0,
                this._stopBatching();
            ...
            _init: function(options) {
                ...
                this._startBatching(),
                options.existingState || this._startLogSession(),
                this._logInitializedEvent()
            },
            _initOptions: function(options) {
                options.existingState ? this._restore(options.existingState) : this._initState(),
                this._initProperties(options)
            },
            _initState: function() {
                var state = {};
                state.sequenceNumber = 0,
                state.lastIncrementingBits = 0,
                state.pending = {},
                state.current = {},
                state.snapshots = [],
                state.currentDelta = [],
            ...
            _initProperties: function(options) {
                for (var option in this)
                    "function" != typeof this[option] && options && "_" !== option.charAt(0) && (void 0 !== options[option] ? this[option] = options[option] : this[option] = this[option])
            },
            ...
            _initContext: function(type, data) {
                var context;
                return context = data ? this._copyData(data) : {},
                SCHEMA && SCHEMA.types[type[0]] ? context.type = SCHEMA.types[type[0]] : context.type = type,
                context.id = this._getNextContextId(),
                context
            },
            _initEventContext: function(type, data) {
                var context = this._initContext(type, data);
                return context.sequence = ++this._state.sequenceNumber,
                void 0 === context.time && (context.time = this._timestamp()),
                context
            },
            ...
            _getNextContextId: function() {
                ...
                return incrementingBits <= this._state.lastIncrementingBits && (incrementingBits = this._state.lastIncrementingBits + 1),
                this._state.lastIncrementingBits = incrementingBits,
                incrementingBits * bitsShift + randomBits
            },
            _snapshot: function(context) {
                ...
                this._state.pending = {},
                this._state.currentDelta.push(count),
                this._state.currentDelta = [],
                this._state.snapshots.push(this._state.currentDelta),
                arguments.length && this._state.currentDelta.push.apply(this._state.currentDelta, arguments),
                this._state.snapshots.length >= this.batchSize && this.flush()
            },
    ...
    require.define("/node_modules/nf-cl-schema-ui/package.json", function(require, module, exports, __dirname, __filename, process, global) {
        module.exports = {
            main: "dist/schema/nf-cl-schema-netflixApp.js"
        }
    }),
    require.define("/node_modules/nf-cl-schema-ui/dist/schema/nf-cl-schema-netflixApp.js", function(require, module, exports, __dirname, __filename, process, global) {
        module.exports = {
            version: "1.197.0",
            name: "netflixApp",
            types: {
                ...
                "android.LiteTwentyProductMode": ["android.LiteTwentyProductMode", "android.LiteProductMode"],
    ...
    require.define("/app/ui.js", function(require, module, exports, __dirname, __filename, process, global) {
        function convertSpeed(result) {
            var bitsPerS = result.speed
              , speedMbs = bitsPerS / 1e3 / 1e3 || 0
              , units = "Mbps";
            return speedMbs < 1 ? (speedMbs *= 1e3,
            units = "Kbps") : speedMbs >= 995 && (speedMbs /= 1e3,
            units = "Gbps"),
            speedMbs = speedMbs < 9.95 ? (Math.round(10 * speedMbs) / 10).toFixed(1) : speedMbs < 100 ? Math.round(speedMbs) : 10 * Math.round(speedMbs / 10),
            {
                speed: speedMbs,
                units: units
            }
        ...
        function convertToMB(result) {
            ...
            return mb = mb > 1 ? mb < 10 ? (Math.round(10 * mb) / 10).toFixed(1) : 10 * Math.round(mb / 10) : (Math.round(100 * mb) / 100).toFixed(2)
        }
        function convertLatency(result) {
            var units = "ms"
              , latency = result.value;
            return 0 !== latency && !latency && result.latency && (latency = result.latency.value),
            0 !== latency && !latency || latency > 1e6 ? {
                speed: 0,
                units: units
            } : (latency >= 1e3 ? (latency = (Math.round(latency / 1e3 * 10) / 10).toFixed(1),
            units = "s") : latency = Math.round(latency),
            {
                speed: latency,
                units: units
            })
        ...
        var TEST_CONFIG = {
          ...
          , ui = function(tester, logger) {
            function getElement(elementId) {
                var element;
                return element || (elemCache[elementId] = element = document.getElementById(elementId)),
                element
            }
            ...
            function removeClass(element, className) {
                return element.className = element.className.replace(new RegExp("(\\s|^)" + className + "(\\s|$)","g"), " "),
                element
            }
            function addClass(element, className) {
                element.className;
                return hasClass(element, className) || (element.className += " " + className),
                element
            }
            function hasClass(element, className) {
                return -1 !== element.className.search(new RegExp("(\\s|^)" + className + "(\\s|$)"))
            }
            ...
            function localizePage(language, localized) {
                var i, key, elem, localizedElems = document.getElementsByClassName("localized"), alignElems = document.getElementsByClassName("align-container"), localizedLang = localized[language], rightAligned = localizedLang.rightAligned, messageContentElem = getElement("share-msg");
                for (i = 0; i < localizedElems.length; i++)
                    elem = localizedElems[i],
                    key = elem.getAttribute("loc-str"),
                    elem.innerHTML = localizedLang[key];
                for (messageContentElem.setAttribute("my-speed", localizedLang.my_internet_speed),
                messageContentElem.setAttribute("your-speed", localizedLang.your_internet_speed),
                messageContentElem.setAttribute("yours-speed", localizedLang.how_fast_is_yours),
                messageContentElem.setAttribute("share-on-fb", localizedLang.share_on_facebook),
                messageContentElem.setAttribute("share-on-tw", localizedLang.share_on_twitter),
                i = 0; i < alignElems.length; ++i)
                    elem = alignElems[i],
                    rightAligned ? addClass(elem, "right-aligned-text") : removeClass(elem, "right-aligned-text")
            }
            function setupHelp() {
                ...
                utils.addEventListener(testHelpElem, "click", function() {
                    testHelpElem.active ? (removeClass(testHelpElem.children[0], "active"),
                    testHelpElem.active = !1,
                    hideElement(helpContentElem)) : (addClass(testHelpElem.children[0], "active"),
                    testHelpElem.active = !0,
                    showElement(helpContentElem),
                    setTimeout(function() {
                        scrollToElement(testHelpElem)
                    }, 0),
                    logger.logEvent("HelpPresented", {
                        view: "Help"
                    }))
                })
            }
            function setupLanguageControls() {
                function toggleSelector() {
                    ...
                    toggleClass(languageSelector, "show"),
                    toggleClass(languageSelectorIcon, "oc-icon-keyboard_arrow_down"),
                    toggleClass(languageSelectorIcon, "oc-icon-keyboard_arrow_up")
                }
                ...
                getElement("help-content"),
                getElement("your-speed-message"),
                getElement("compare-on");
                utils.addEventListener(languageSelectorBtn, "click", toggleSelector),
                ...
                for (elemInd = 0; elemInd < languageOptions.length; ++elemInd)
                    elem = languageOptions[elemInd],
                    function(elem) {
                        utils.addEventListener(elem, "click", function(event) {
                            ...
                            if (window.localized)
                                localizePage(langPath, window.localized),
                                utils.removeEventListener(languageSelectorBtn, toggleSelector),
                                languageSelectorBtn.innerHTML = languageSelectorBtn.innerHTML.replace(languageSelectorBtn.innerText, language),
                                utils.addEventListener(languageSelectorBtn, "click", toggleSelector),
                                curLang = langPath;
                            else {
                                ...
                                xhr.onreadystatechange = function(e) {
                                    4 !== this.readyState || 200 !== this.status && 304 !== this.status || (window.localized = JSON.parse(this.responseText),
                                    localizePage(langPath, window.localized),
                                    utils.removeEventListener(languageSelectorBtn, toggleSelector),
                                    languageSelectorBtn.innerHTML = languageSelectorBtn.innerHTML.replace(languageSelectorBtn.innerText, language),
                                    utils.addEventListener(languageSelectorBtn, "click", toggleSelector),
                                ...
                                ,
                                (!window.location || "localhost" != window.location.hostname && "https:" !== window.location.protocol) && (path = "https://fast.com" + path),
                                xhr.open("GET", path, !0),
                                xhr.onerror = function(e) {}
                                ,
                                logger.logEvent("LanguageChangeStart", {
                                    from: curLang,
                                    to: langPath
                                }),
                                setTimeout(function() {
            ...
            function pause() {
                tester.isRunning() && tester.stop(),
                utils.addEventListener(pauseElem, "click", restartTest)
            }
            function setupPause() {
                utils.removeEventListener(pauseElem, "click", restartTest),
                utils.removeEventListener(pauseElem, "click", pause),
                utils.addEventListener(pauseElem, "click", pause)
            }
            ...
            function setupAfterTestActions() {
                ...
                showElement(actionsElem),
                showElement(speedMsg),
                showElement(resultsExplanationElem),
                showingMoreDetails || showElement(showMoreDetailsBtn)
            }
            function reset() {
                ...
                hideElement(actionsElem),
                hideElement(unstableResultsElem),
                hideElement(testErrorElem),
                hideElement(speedMsg),
                hideElement(showMoreDetailsBtn),
                speedElem.innerHTML = 0,
                speedUnitsElem.innerHTML = "&nbsp",
                removeClass(detailsElem, "succeeded"),
                removeClass(latencyContainer, "succeeded"),
                removeClass(speedElem, "succeeded"),
                removeClass(speedElem, "failed"),
                removeClass(speedUnitsElem, "succeeded"),
                removeClass(speedUnitsElem, "failed"),
                removeClass(speedProgressIndicator, "succeeded"),
                removeClass(speedProgressIndicator, "stopped"),
                removeClass(speedProgressIndicator, "failed"),
                addClass(speedProgressIndicator, "in-progress"),
                removeClass(speedProgressIndicatorIcon, "oc-icon-refresh"),
                addClass(speedProgressIndicatorIcon, "oc-icon-pause"),
                removeClass(infoElem, "succeeded"),
                removeClass(infoElem, "failed"),
                removeClass(latencyElem, "succeeded"),
                removeClass(latencyElem, "failed"),
                removeClass(latencyUnitsElem, "succeeded"),
                removeClass(latencyUnitsElem, "failed"),
                removeClass(latencyLabelElem, "succeeded"),
                removeClass(latencyLabelElem, "failed"),
                removeClass(bufferbloatElem, "succeeded"),
                removeClass(bufferbloatElem, "failed"),
                removeClass(bufferbloatUnitsElem, "succeeded"),
                removeClass(bufferbloatUnitsElem, "failed"),
                removeClass(bufferbloatLabelElem, "succeeded"),
                removeClass(bufferbloatLabelElem, "failed"),
                removeClass(uploadElem, "succeeded"),
                removeClass(uploadElem, "failed"),
                removeClass(uploadUnitsElem, "succeeded"),
                removeClass(uploadUnitsElem, "failed"),
                removeClass(uploadLabelElem, "succeeded"),
                removeClass(uploadLabelElem, "failed"),
                latencyElem.innerHTML = "0",
                bufferbloatElem.innerHTML = "0",
                uploadElem.innerHTML = "0",
                bytesDownElem.innerHTML = "0",
                bytesUpElem.innerHTML = "0"
            }
            function scrollToElement(elem) {
                elem.scrollIntoView ? elem.scrollIntoView() : window.scroll(0, function(obj) {
                    var curtop = 0;
                    if (obj.offsetParent) {
                        do {
            ...
            var fbShareEventListener, twShareEventListener, fbShareBtn, twShareBtn, showMoreDetailsBtn, testAgainBtn, resultsExplanationElem, configContainerElem, detailsElem, latencyContainer, pauseElem, downloadResult, showingMoreDetails, config, elemCache = {}, utils = require("./utils"), events = require("./event").events, curLang = document.body.getAttribute("language"), canUseLocalStorage = !1;
            try {
                window && window.localStorage && (window.localStorage.__test__ = 1,
                canUseLocalStorage = !0)
            } catch (e) {}
            var showMoreDetails = function() {
                if (logger.logEvent("AdvancedMetricsPresented"),
                tester.isRunning()) {
                ...
                showingMoreDetails = !0,
                hideElement(showMoreDetailsBtn),
                showElement(detailsElem)
            }
              , hideMoreDetails = function() {
                hideElement(detailsElem),
                showElement(showMoreDetailsBtn),
                showingMoreDetails = !1
            }
              , updateClientInfo = function(event) {
                var locationsStr, curLocation, i, client = event.client, locationsMap = {};
                for (getElement("user-ip").innerHTML = client.ip,
                client.isp ? getElement("user-isp").innerHTML = client.isp.replace(/_/g, " ") : getElement("user-isp").innerHTML = "",
                getElement("user-location").innerHTML = client.location.city + ", " + client.location.country,
                curLocation = client.servers[0].location.city + ", " + client.servers[0].location.country,
                locationsStr = curLocation,
                locationsMap[curLocation] = !0,
                i = 1; i < Math.min(client.servers.length, 3); ++i)
                    curLocation = client.servers[i].location.city + ", " + client.servers[i].location.country,
                    locationsMap[curLocation] || (locationsStr += "&nbsp&nbsp|&nbsp&nbsp" + curLocation,
                    locationsMap[curLocation] = !0);
                getElement("server-locations").innerHTML = locationsStr
            }
              , showConfig = function() {
                logger.logEvent("SettingsPresented"),
                renderConfig(config),
                hideElement(detailsElem),
                showElement(configContainerElem)
            }
              , cancelConfig = function() {
                hideElement(configContainerElem),
                !0 === config.showAdvanced || "true" === config.showAdvanced ? showMoreDetails() : hideMoreDetails()
            }
              , applyConfig = function() {
                ...
                for (var name in TEST_CONFIG) {
                    ...
                    testerValue && ("boolean" == typeof TEST_CONFIG[name].default ? config[name] = testerValue.checked : config[name] = testerValue.value,
                    shouldPersist && canUseLocalStorage && (window.localStorage[name] = String(config[name])))
                }
                try {
              ...
              , resetConfig = function() {
                var defaultConfig = {};
                for (var name in TEST_CONFIG)
                    defaultConfig[name] = TEST_CONFIG[name].default,
                    canUseLocalStorage && delete window.localStorage[name];
                applyTesterConfig(defaultConfig),
              ...
              , renderConfig = function(config) {
                for (var name in TEST_CONFIG) {
                    var testerValue = getElement(TEST_CONFIG[name].elem);
                    "boolean" == typeof TEST_CONFIG[name].default ? testerValue.checked = !0 === config[name] || "true" === config[name] : testerValue.value = config[name]
                }
            }
              , getTestSettings = function(tester) {
                ...
                if (canUseLocalStorage)
                    for (var name in TEST_CONFIG) {
                        var testerValue;
                        TEST_CONFIG[name].testerFunc && (testerValue = TEST_CONFIG[name].testerFunc(testerConfig)),
                        config[name] = window.localStorage[name] || testerValue || TEST_CONFIG[name].default
                    }
                return config
            }
              , applyTesterConfig = function(config) {
                ...
                tester.setConfig(testerConfig),
                testerConfig
            }
              , that = {
                onProgress: function(result) {
                    var updateMetric = function(testType) {
                        var speedElemId, speedUnitId, speedData, speedElem, speedUnitsElem, bytesElemId, bytesElem, convertFunc;
                        "download" == testType ? (speedElemId = "speed-value",
                        bytesElemId = "down-mb-value",
                        speedUnitId = "speed-units",
                        convertFunc = convertSpeed) : "upload" == testType ? (speedElemId = "upload-value",
                        bytesElemId = "up-mb-value",
                        speedUnitId = "upload-units",
                        convertFunc = convertSpeed) : "latency" == testType ? (speedElemId = "latency-value",
                        speedUnitId = "latency-units",
                        convertFunc = convertLatency) : "bufferbloat" == testType && (speedElemId = "bufferbloat-value",
                        speedUnitId = "bufferbloat-units",
                        convertFunc = convertLatency),
                        speedElem = getElement(speedElemId),
                        speedUnitsElem = getElement(speedUnitId),
                        bytesElemId && (bytesElem = getElement(bytesElemId),
                        bytesElem.innerHTML = convertToMB(result)),
                        speedData = convertFunc(result),
                        speedData.speed,
                        speedData.units,
                        speedElem && (speedElem.innerHTML = speedData.speed),
                        speedUnitsElem && (speedUnitsElem.innerHTML = speedData.units)
                    };
                    updateMetric(result.testType),
                    "download" === result.testType && updateMetric("bufferbloat"),
                    "true" !== config.measureUploadLatency && !0 !== config.measureUploadLatency || "upload" !== result.testType || updateMetric("bufferbloat")
                },
                onComplete: function(result) {
                    var className, speedElemId, speedUnitId, speedLabelId, speedElem, speedUnitsElem, speedLabelElem, unstableResultsElem, testErrorElem, testType = result.testType, speedProgressIndicator = getElement("speed-progress-indicator"), speedProgressIndicatorIcon = getElement("speed-progress-indicator-icon"), showAfterTestActions = !1;
                    if (downloadResult = result,
                    "download" == testType ? (speedElemId = "speed-value",
                    speedUnitId = "speed-units",
                    speedLabelId = "speed-value",
                    showAfterTestActions = !0,
                    convertFunc = convertSpeed) : "upload" == testType ? (speedElemId = "upload-value",
                    speedUnitId = "upload-units",
                    speedLabelId = "upload-label",
                    convertFunc = convertSpeed) : "latency" == testType ? (speedElemId = "latency-value",
                    speedUnitId = "latency-units",
                    speedLabelId = "latency-label",
                    convertFunc = convertLatency) : "bufferbloat" == testType && (speedElemId = "bufferbloat-value",
                    speedUnitId = "bufferbloat-units",
                    speedLabelId = "bufferbloat-label",
                    convertFunc = convertLatency),
                    speedElem = getElement(speedElemId),
                    speedUnitsElem = getElement(speedUnitId),
                    speedLabelElem = getElement(speedLabelId),
                    "success" === result.result ? (that.onProgress(result),
                    result.stable ? className = "succeeded" : (className = "failed",
                    unstableResultsElem = getElement("unstable-results-msg"),
                    showElement(unstableResultsElem))) : "stop" !== result.result ? (testErrorElem = getElement("error-results-msg"),
                    showElement(testErrorElem),
                    className = "failed") : (className = "stopped",
                    showAfterTestActions = !1),
                    showAfterTestActions && setupAfterTestActions(),
                    ("download" === testType && !showingMoreDetails || "upload" === testType || "stop" === result.result) && (removeClass(speedProgressIndicator, "in-progress"),
                    removeClass(speedProgressIndicatorIcon, "oc-icon-pause"),
                    addClass(speedProgressIndicatorIcon, "oc-icon-refresh"),
                    addClass(speedProgressIndicator, className),
                    utils.removeEventListener(testAgainBtn, "click", pause),
                    utils.addEventListener(testAgainBtn, "click", restartTest)),
                    addClass(speedElem, className),
                    addClass(speedUnitsElem, className),
                    addClass(speedLabelElem, className),
                    logger.flush(),
                    "download" === testType && "success" === result.result) {
                        var speedData = convertSpeed(result);
                        fbShareBtn = getElement("share-on-facebook-link"),
                        twShareBtn = getElement("share-on-twitter-link"),
                        fbShareEventListener && utils.removeEventListener(fbShareBtn, "click", fbShareEventListener),
                        twShareEventListener && utils.removeEventListener(fbShareBtn, "click", twShareEventListener),
                        fbShareEventListener = function() {
                            var location, targetUrl, facebookUrl, hostname, protocol, messageContentElem = getElement("share-msg"), shareDescription = messageContentElem.getAttribute("my-speed") + speedData.speed + speedData.units + ". " + messageContentElem.getAttribute("yours-speed"), shareTitle = messageContentElem.getAttribute("share-on-fb");
                            location = window.location,
                            hostname = location.hostname,
                            protocol = location.protocol,
                            "http:" !== location.protocol && "https:" !== location.protocol && (hostname = "fast.com",
                            protocol = "https:"),
                            targetUrl = protocol + "//" + hostname + "/" + curLang + "/share/" + speedData.speed + speedData.units + ".html",
                            facebookUrl = "https://www.facebook.com/sharer/sharer.php?u=" + encodeURI(targetUrl) + "&title=" + encodeURI("FAST speed test") + "&description=" + encodeURI(shareDescription),
                            logger.logEvent("FacebookShare", {
                                url: targetUrl
                            }),
                        ...
                        twShareEventListener = function() {
                            var location, targetUrl, twitterUrl, hostname, protocol, messageContentElem = getElement("share-msg"), shareDescription = messageContentElem.getAttribute("my-speed") + " " + speedData.speed + speedData.units + ". " + messageContentElem.getAttribute("yours-speed"), shareTitle = messageContentElem.getAttribute("share-on-tw");
                            location = window.location,
                            hostname = location.hostname,
                            protocol = location.protocol,
                            "http:" !== location.protocol && "https:" !== location.protocol && (hostname = "fast.com",
                            protocol = "https:"),
                            targetUrl = protocol + "//" + hostname + "/" + curLang + "/share/" + speedData.speed + speedData.units + ".html",
                            twitterUrl = "https://twitter.com/intent/tweet?url=" + encodeURI(targetUrl) + "&text=" + encodeURI(shareDescription),
                            twitterUrl = twitterUrl.replace(/;/g, "%3B"),
                            logger.logEvent("TwitterShare", {
                                url: targetUrl
                            }),
                            popupCenter(twitterUrl, shareTitle, 400, 400)
                        }
                        ,
                        utils.addEventListener(fbShareBtn, "click", fbShareEventListener),
                        utils.addEventListener(twShareBtn, "click", twShareEventListener);
                        var infoElem = getElement("test-info-container")
                          , showInfo = function(uploadResult) {
                            tester.off(events.END, showInfo),
                            "stop" !== uploadResult.result && (addClass(infoElem, "succeeded"),
                            addClass(detailsElem, "succeeded"))
                        }
                          , doUpload = function(latencyResult) {
                            tester.off(events.END, doUpload),
                            "stop" !== latencyResult.result && (addClass(latencyContainer, "succeeded"),
                            !downloadResult || void 0 !== downloadResult.latency && null !== downloadResult.latency || (downloadResult.latency = downloadResult.latency || {
                                value: "NA"
                            }),
                            that.onComplete({
                            ...
                            tester.on(events.END, showInfo),
                            tester.upload())
                        };
                        tester.on(events.END, doUpload),
                        tester.latency()
                    }
                },
                setupEvents: function() {
                    ...
                    renderConfig(config),
                    pauseElem = getElement("speed-progress-indicator"),
                    fbShareBtn = getElement("share-on-facebook-link"),
                    twShareBtn = getElement("share-on-twitter-link"),
                    testAgainBtn = getElement("speed-progress-indicator"),
                    showMoreDetailsBtn = getElement("show-more-details-link"),
                    cancelConfigBtn = getElement("cancel-config"),
                    applyConfigBtn = getElement("apply-config"),
                    resetConfigBtn = getElement("reset-config"),
                    testConfigBtn = getElement("settings-link"),
                    resultsExplanationElem = getElement("test-context-container"),
                    configContainerElem = getElement("test-config-container"),
                    detailsElem = getElement("extra-details-container"),
                    latencyContainer = getElement("latency-container"),
                    setupHelp(),
                    setupPause(),
                    setupLanguageControls(),
                    utils.addEventListener(showMoreDetailsBtn, "click", showMoreDetails),
                    utils.addEventListener(testConfigBtn, "click", showConfig),
                    utils.addEventListener(cancelConfigBtn, "click", cancelConfig),
                    utils.addEventListener(applyConfigBtn, "click", applyConfig),
                    utils.addEventListener(resetConfigBtn, "click", resetConfig),
                    tester.on(events.URL_REQUEST_END, updateClientInfo),
                    !0 !== config.showAdvanced && "true" !== config.showAdvanced || showMoreDetails()
                }
            };
            return that
        };
    ...
    require.define("/app/viewport-units-buggyfill.js", function(require, module, exports, __dirname, __filename, process, global) {
        !function(root, factory) {
            ...
            function initialize(initOptions) {
                if (!initialized) {
                    ...
                    !isMobileSafari && !isBadStockAndroid && !isOperaMini)
                    ...
                    window.addEventListener("orientationchange", _refresh, !0),
                    window.addEventListener("pageshow", _refresh, !0),
                    refresh()
                }
            }
            ...
            function findDeclarations(rule) {
                if (7 === rule.type) {
                    ...
                    return viewportUnitExpression.lastIndex = 0,
                    void (viewportUnitExpression.test(value) && declarations.push([rule, null, value]))
                }
                if (!rule.style) {
                    if (!rule.cssRules)
                        return;
                ...
                forEach.call(rule.style, function(name) {
                    ...
                    rule.style.getPropertyPriority(name) && (value += " !important"),
                    viewportUnitExpression.lastIndex = 0,
                    viewportUnitExpression.test(value) && declarations.push([rule, name, value])
                })
            }
            function getReplacedViewportUnits() {
                ...
                return declarations.forEach(function(item) {
                    ...
                    if (!_open || _open !== open)
                        return buffer.length && (css.push(open + buffer.join("\n") + close),
                        buffer.length = 0),
                        void (_open ? (open = _open,
                        close = _close,
                        buffer.push(_item.content)) : (css.push(_item.content),
                        open = null,
                        close = null));
                    _open && !open && (open = _open,
                    close = _close),
                    buffer.push(_item.content)
                }),
                buffer.length && css.push(open + buffer.join("\n") + close),
                isOperaMini && css.push("* { content: normal !important; }"),
                css.join("\n\n")
            }
            function overwriteDeclaration(rule, name, value) {
                var _value, _selectors = [];
                _value = value.replace(viewportUnitExpression, replaceValues),
                name && (_selectors.push(rule.selectorText),
                _value = name + ": " + _value + ";");
            ...
            function replaceValues(match, number, unit) {
                ...
                return parseFloat(number) / 100 * _base + "px"
            }
            ...
            var options, dimensions, declarations, styleNode, initialized = !1, userAgent = window.navigator.userAgent, viewportUnitExpression = /([+-]?[0-9.]+)(vh|vw|vmin|vmax)/g, forEach = [].forEach, isOperaMini = userAgent.indexOf("Opera Mini") > -1, isMobileSafari = /(iPhone|iPod|iPad).+AppleWebKit/i.test(userAgent) && function() {
                var iOSversion = userAgent.match(/OS (\d)/);
                return iOSversion && iOSversion.length > 1 && parseInt(iOSversion[1]) < 10
            }(), isBadStockAndroid = function() {
                return userAgent.indexOf(" Android ") > -1 && (userAgent.indexOf("Version/") > -1 && parseFloat((userAgent.match("Android ([0-9.]+)") || [])[1]) <= 4.4)
            }();
            return {
                version: "0.6.0",
    ...
    require.define("/app/browser_test.js", function(require, module, exports, __dirname, __filename, process, global) {
        ...
        window.onerror = function(errorMsg, url, lineNumber, column, errorObj) {
            ...
            l.logEvent("ExceptionOccurred", errorData),
            flushLogger(!0))
        }
        ,
        aggregator = require("./aggregator/stableMovingAverage")(5),
        stopper = require("./stopper/stableDeltaMeasurementsStopper")({
            minDuration: 7,
            maxDuration: 30,
            stabilityDelta: 2,
        ...
        testerFactory = require("./tester"),
        utils = require("./utils"),
        events = require("./event").events,
        requester = require("./requester/xhr"),
        logging = require("./logger"),
        apiEndpoint = "api.fast.com/netflix/speedtest/v2",
        loggingEndpoint = "https://ichnaea-web.netflix.com/cl2",
        window.console || (console = {
            log: utils.dummy
        }),
        utils.polyfillObjectKeys(),
        tester = testerFactory(requester, {
            collectAfterComplete: !1,
            duration: {
                min: 5,
                max: 30
        ...
        if (testLogger.logEvent("PageVisit"),
        flushLogger(!0),
        setInterval(flushLogger, 2e3),
        window.location.pathname.indexOf("/share/") >= 0) {
            ...
            l.logEvent("ShareLinkClicked", data),
            flushLogger(!0),
            setTimeout(function() {
                window.location.href = location.protocol + "//" + location.hostname
            }, 5)
        }
        ui = require("./ui")(tester, testLogger),
        tester.on(events.START, function() {
